...
class NaiveBayesClassifier(object):
    ...
    def compute_BoW_vector(self, sentence):
        BoW_vector = [0] * len(self.V)
        for word in sentence:
            if word in self.V:
                BoW_vector[self.V[word]] = 1
        return BoW_vector

    def train(self, training_sentences, training_labels):
        # Tokenize and compute BoW representation for each sentence with vocabulary used in training set
        training_set = self.word_tokenization_dataset(training_sentences)
        self.V = self.compute_vocabulary(training_set)
        training_BoW_vectors = [self.compute_BoW_vector(sentence) for sentence in training_set]

        # Calculate class prior probabilities
        total_sentences = len(training_sentences)
        class_sentence_counts = Counter(training_labels)
        self.prior = {label : count / total_sentences for label, count in class_sentence_counts.items()}

        # Initialize conditional probability matrix
        num_classes = len(set(training_labels))
        num_words = len(self.V)
        self.conditional = np.zeros((num_classes, num_words))

        # Count number of sentences in which each word occurs by class
        class_word_counts = {label: np.zeros(num_words) for label in set(training_labels)}
        for BoW_vector, label in zip(training_BoW_vectors, training_labels):
            class_word_counts[label] += BoW_vector

        # Calculate conditional probabilities for each word given each class
        for label, class_word_count in sorted(class_word_counts.items()):
            total_words_in_class = class_sentence_counts[label]
            if total_words_in_class > 0:
                # Calculate conditional probability for each word in the given class
                # Replace the entire row for that class with these conditional probabilities
                self.conditional[label, :] = class_word_count / total_words_in_class

    def predict(self, test_sentence):
        # Initialize log probabilities for each class of the test sentence
        label_probability = {
            0: 0,
            1: 0,
            -1:0,
        ...
        # Near zero value used as to avoid log(0) in log-probability calculations
        epsilon = 1e-10
        ...
        test_BoW_vector = self.compute_BoW_vector(test_sentence)

        # Calculate log-probabilities for each class
        for label, class_prob in sorted(self.prior.items()):
            log_prob = np.log(class_prob)
            for idx, word_presence in enumerate(test_BoW_vector):
                if word_presence == 0: continue
                log_prob += np.log(self.conditional[label][idx] if self.conditional[label][idx] > 0 else epsilon)
